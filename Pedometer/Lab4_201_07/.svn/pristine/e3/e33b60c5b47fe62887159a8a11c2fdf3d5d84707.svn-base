...
public class MapView extends View {
	...
	public final int[] defaultColors = { 0xffffffff,// 0xff000000, // lines
...
//			if (MainActivity.map1.calculateIntersections(userPath.get(i),
//					userPath.get(i + 1)).isEmpty()) {
		...
		float cosAngle = (float) Math.cos(MainActivity.azimuth - Math.PI / 2);
		float sinAngle = (float) Math.sin(MainActivity.azimuth - Math.PI / 2);
		canvas.drawLine((userPoint.x) * scale.x - (float) 0.1 * scale.x
				* sinAngle, (userPoint.y) * scale.y + (float) 0.1 * scale.y
				* cosAngle, (userPoint.x) * scale.x + (float) 0.8 * scale.x
				* cosAngle, (userPoint.y) * scale.y + (float) 0.8 * scale.y
				* sinAngle, linePaints.get(USER_POINT_COLOR_INDEX));
		canvas.drawLine((userPoint.x) * scale.x + (float) 0.1 * scale.x
				* sinAngle, (userPoint.y) * scale.y - (float) 0.1 * scale.y
				* cosAngle, (userPoint.x) * scale.x + (float) 0.8 * scale.x
				* cosAngle, (userPoint.y) * scale.y + (float) 0.8 * scale.y
